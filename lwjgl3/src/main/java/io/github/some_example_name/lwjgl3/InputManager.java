...
public class InputManager implements IInputSystem {
    ...
    public InputManager(InputBindings bindings) {
        ...
        for (InputAction action : InputAction.values()) actionTriggered.put(action, false);
    }
    ...
    public float getMouseY() { return Gdx.graphics.getHeight() - Gdx.input.getY(); }

    /** Convenience: raw key check. */
    public boolean isKeyDown(int keycode) { return Gdx.input.isKeyPressed(keycode); }
